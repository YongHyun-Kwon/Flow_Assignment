package com.flow.assignment.web;

import com.flow.assignment.domain.extension.CustomExtension;
import com.flow.assignment.domain.extension.CustomExtensionRepository;
import com.flow.assignment.domain.extension.ExtensionRepository;
import com.flow.assignment.web.dto.ExtensionRequestDto;
import org.junit.After;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.boot.test.web.client.TestRestTemplate;
import org.springframework.boot.test.web.server.LocalServerPort;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.test.context.junit4.SpringRunner;

import java.util.List;

import static org.assertj.core.api.Assertions.assertThat;

@RunWith(SpringRunner.class)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class ExtensionApiControllerTest {

    @LocalServerPort
    private int port;

    @Autowired
    private TestRestTemplate restTemplate;

    @Autowired
    private ExtensionRepository extensionRepository;

    @Autowired
    private CustomExtensionRepository customExtensionRepository;

    @After
    public void tearDown() throws Exception {
        extensionRepository.deleteAll();
        customExtensionRepository.deleteAll();
    }

    @Test
    public void saveTest() throws Exception {

        String name = "js";
        ExtensionRequestDto extensionRequestDto = ExtensionRequestDto.builder().name(name).build();

        String url = "http://localhost:" + port + "/api/v1/add-extension";

        ResponseEntity<Long> responseEntity = restTemplate.postForEntity(url, extensionRequestDto, Long.class);

        assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(responseEntity.getBody()).isGreaterThan(0L);

        List<CustomExtension> all = customExtensionRepository.findAll();
        assertThat(all.get(0).getName()).isEqualTo(name);
    }
}
